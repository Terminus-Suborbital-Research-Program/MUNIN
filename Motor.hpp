...
class Motor
{
    ...
    struct PID
    {
        ...
        PID(double P, double I, double D, MotorClock& clock, int& steps, std::chrono::microseconds output_max);
    ...
    bool m_driving, m_using_pid;

    const unsigned int m_resolution;
    int m_microstep_point, m_revs, m_microsteps, m_step_setpoint;
    ...
    */
    ...
    /*
    EXTREMELY IMPORTANT TO CALL THE DECONSTRUCTOR WHEN FINISHED. If not, ownereship of the gpio pins will remain transferred and resurces will remain allocated.
    */
    ~Motor();
